...
public sealed record ChangeWorkerPasswordDto(string Email, string OldPassword, string NewPassword, string RepeatPassword);

public sealed class ChangeWorkerPasswordDtoValidator : AbstractValidator<ChangeWorkerPasswordDto>
{
	public ChangeWorkerPasswordDtoValidator(IWorkerRepository repository, IPasswordHasher<User> passwordHasher)
	{
		RuleFor(r => r.Email)
			.NotEmpty()
			.EmailAddress()
